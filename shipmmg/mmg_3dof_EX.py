...
def simulate(
    ...
    **options
    ...
    def mmg_3dof_eom_solve_ivp(t, X):
        ...
        # X_F0_initial = 0.0
        # X_F0_final = 3.0
        # X_F0 = X_F0_initial + (X_F0_final - X_F0_initial) /150 *t
        # Y_F0 = 0.0
       
        # 外力がガウス分布に従う場合
        # X_F0 = 3.0 * np.exp(-(t-75)**2/100)
        # Y_F0 = 0.0
        
        #外力がサイン波に従う場合
        # X_F0 = 0.5 * np.sin(2*np.pi/5*t) + 0.7
        # # X_F0 = 1.5 * np.sin(2*np.pi/5*t) + 2.0
        # Y_F0 = 0.0
        
        #斜め方向(y=x)にsin波がかかる場合
        # X_F0 = 1.5 * np.sin(2*np.pi/15*t) + 2.0
        # Y_F0 = 1.5 * np.sin(2*np.pi/15*t) + 2.0
        ...
        # amplitude = 0.3  # 波の振幅
        # # frequency = np.linspace(0.05, 0.2, num_waves)  # 波の周波数
        # # frequency = np.linspace(0.5, 1.0, num_waves)  # 波の周波数
        # frequency = np.linspace(0.1, 0.7, num_waves)  # 波の周波数
        # np.random.seed(0)
        # phase = np.random.rand(num_waves) * 2 * np.pi  # ランダムな位相
        # wave = 0.0
        # # 不規則波の生成
        # for i in range(num_waves):
        #     wave += amplitude * np.sin(2 * np.pi * frequency[i] * t + phase[i]) + 1/20  
        # X_F0 = wave
        # Y_F0 = 0.0      
        
        X_F = np.cos(ψ) * X_F0 + np.sin(ψ) * Y_F0
        Y_F = -np.sin(ψ) * X_F0 + np.cos(ψ) * Y_F0
        N_F = Y_F * np.abs(x_G)

        # d_u = ((X_H + X_R + X_P + X_F) + (m + m_y) * v * r + x_G * m * (r**2)) / (m + m_x)
        # d_v = (
        #     (x_G**2) * (m**2) * u * r
        #     - (N_H + N_R + N_F) * x_G * m
        #     + ((Y_H + Y_R + Y_F) - (m + m_x) * u * r) * (I_zG + J_z + (x_G**2) * m)
        # ) / ((I_zG + J_z + (x_G**2) * m) * (m + m_y) - (x_G**2) * (m**2))
        # d_r = (N_H + N_R + N_F - x_G * m * (d_v + u * r)) / (I_zG + J_z + (x_G**2) * m)

        d_u = ((X_H + X_R + X_P + X_F) + (m + m_y) * v * r) / (m + m_x)
        d_v = ((Y_H + Y_R + Y_F) - (m + m_x) * u * r) / (m + m_y)
        d_r = (N_H + N_R + N_F) / (I_zG + J_z)
